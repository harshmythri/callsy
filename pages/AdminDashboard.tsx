...
const AdminDashboard: React.FC = () => {
  ...
  const provisionBusiness = () => {
    ...
    if (newBizCustomId.trim()) {
      ...
      if (finalId.length < 1) {
        setProvisionError('Invalid Custom ID.');
        return;
      }
      const exists = businesses.some(b => b.id === finalId);
      if (exists) {
        setProvisionError(`ID Conflict: "${finalId}" already exists.`);
        return;
      }
    } else {
      const baseSlug = sanitizeId(newBizName);
      const randomSuffix = Math.floor(100 + Math.random() * 899);
      finalId = baseSlug ? `${baseSlug}-${randomSuffix}` : `biz-${randomSuffix}`;
    }
      ...
      <div className="max-w-7xl mx-auto py-10 px-6">
            ...
            <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 border border-slate-200">
              ...
              <div className="space-y-6">
                ...
                <div className="pt-4 flex gap-3">
                  <button onClick={() => setIsAuditModalOpen(false)} className="flex-1 py-3 text-slate-500 text-sm font-bold hover:bg-slate-50 rounded-xl">Close</button>
                  <button onClick={() => {
                    setBusinesses(prev => prev.map(b => b.id === selectedBusiness.id ? selectedBusiness : b));
                    setIsAuditModalOpen(false);
                  }} className="flex-1 py-3 bg-slate-900 text-white text-sm font-bold rounded-xl shadow-lg hover:bg-slate-800 active:scale-95">Update Registry</button>
                </div>
              </div>
            </div>
          </div>
              ...
              {!provisionSuccessId ? (
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="flex items-center">
                        <span className="bg-slate-100 border border-r-0 border-slate-200 px-3 py-3 rounded-l-xl text-slate-400 text-xs font-medium truncate max-w-[120px]">
                          {currentOrigin.split('//')[1]}/#/call/
                        </span>
                        <input 
                          type="text" 
                          ...
                          className="flex-grow px-4 py-3 bg-slate-50 border border-slate-200 rounded-r-xl outline-none focus:border-blue-500 transition-colors"
                        />
                      </div>
                    </div>
                ...
                <div className="text-center py-6">
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-8 text-left">
                    ...
                    <div className="flex gap-2">
                      <input readOnly value={`${currentOrigin}/#/call/${provisionSuccessId}`} className="flex-grow bg-white border border-slate-200 rounded-lg px-3 py-2 font-mono text-sm text-blue-700 font-bold" />
                      <button onClick={() => navigator.clipboard.writeText(`${currentOrigin}/#/call/${provisionSuccessId}`)} className="px-4 py-2 bg-slate-900 text-white rounded-lg text-xs font-bold">Copy</button>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          {[
            ...
            { label: 'Live Sessions', value: businesses.filter(b => b.presence === 'ONLINE').length.toString(), trend: 'Real-time' },
            ...
            <div key={i} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm text-left">
              ...
              <div className="flex justify-between items-end">
                ...
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${i === 1 ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600'}`}>
                  {stat.trend}
                </span>
              </div>
            </div>
