  ...
  return (
    <Router>
      <div className="min-h-screen flex flex-col bg-white">
        <nav className="bg-white/90 backdrop-blur-md border-b border-slate-100 px-6 py-4 flex justify-between items-center sticky top-0 z-[100]">
          ...
          <div className="hidden md:flex items-center gap-10">
            ...
            {!user ? (
              ...
              <div className="flex items-center gap-6">
                <Link 
                  to={user.role === UserRole.ADMIN ? "/admin8886" : "/dashboard"} 
                ...
                >
                  ...
                  {user.role === UserRole.ADMIN ? 'Admin Panel' : 'Manage Shop'}
                </Link>
                <button 
          ...
          <button onClick={toggleMenu} className="md:hidden p-2 text-slate-600 z-[210]">
            {isMenuOpen ? (
          ...
          {/* Fixed Mobile Menu with solid background and high z-index */}
          <div className={`fixed inset-0 bg-white z-[200] transform transition-transform duration-500 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} md:hidden flex flex-col pt-32 px-10 shadow-2xl`}>
            <div className="flex flex-col gap-10">
              ...
              {!user ? (
                ...
                <>
                  <Link to={user.role === UserRole.ADMIN ? "/admin8886" : "/dashboard"} onClick={closeMenu} className="text-2xl font-black text-blue-700">Dashboard</Link>
                  <button onClick={() => { setUser(null); closeMenu(); }} className="text-left text-2xl font-black text-slate-400">Logout</button>
                </>
              )}
            </div>
          </div>
        ...
        <main className="flex-grow">
          <Routes>
            ...
            <Route path="/admin8886" element={user?.role === UserRole.ADMIN ? <AdminDashboard /> : <Navigate to="/login" />} />
